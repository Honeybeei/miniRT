...
/*   Updated: 2023/02/08 15:40:29 by jchoi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	exp_value(t_figure *fg_, t_pvec3 pos_)
{
	t_vec3	v_;
	double	dval[2];

	if (fg_->type_ == type_sp_)
		return (sqrsum_vec3(sub_vec3(fg_->pos_, pos_)) - pow(fg_->r_, 2));
	else if (fg_->type_ == type_pl_)
		return (dot_product(fg_->dir_, sub_vec3(pos_, fg_->pos_)));
	else if (fg_->type_ == type_cy_)
	{
		v_ = sub_vec3(pos_, fg_->pos_);
		dval[0] = dot_product(v_, fg_->dir_);
		dval[1] = length_vec3(normal_vec3(v_, fg_->dir_));
		if (0 < dval[0] && dval[0] < fg_->h_ && dval[1] < fg_->r_)
			return (1);
		else
			return (-1);
	}
...
void	get_light(t_objs *objs_, size_t i, t_line3 sight_, t_cpnt *contact_)
{
	...
	if (exp_value(contact_->fg_, light_.light_point_)* exp_value(contact_->fg_, contact_->pos_) <= 0)
		return ;
